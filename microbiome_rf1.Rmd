...
```{r}
otu_table_scaled_pct <- data.frame(t(otu_table_scaled), check.names = FALSE)

# Compute NMDS
set.seed(12345); capture.output(otu_table_scaled_pct.mds <- metaMDS(otu_table_scaled_pct, trymax = 200, autotransform = F, wascores = F))

# Permanova
set.seed(12345); otu_table_scaled_pct.perm <- adonis(formula = otu_table_scaled_pct ~ metadata$state) # R2 0.19607, p < 0.001

# Making dataframe for plotting
otu_table_scaled_pct.mds.df <- data.frame(scores(otu_table_scaled_pct.mds, display = 'sites'))
otu_table_scaled_pct.mds.df$state <- metadata$state[match(row.names(otu_table_scaled_pct.mds.df), row.names(metadata))]
...
```{r}
# Plot NMDS
ggplot(otu_table_scaled_pct.mds.df, aes(x=NMDS1, y=NMDS2)) + stat_ellipse(alpha=0.8, aes(color=state), show.legend = F, lwd=0.2) + 
  geom_point(alpha=0.9, aes(fill = state), size=3, color="black", pch=21, stroke=0.2) + scale_fill_manual(values=two_cols) + scale_color_manual(values=two_cols) + 
  labs(title = "Species Level: Beta Diversity Comparison", subtitle = "Relative Abundance", fill="Subject state") +
  annotate("text", x = max(otu_table_scaled_pct.mds.df$NMDS1)-0.2, y = max(otu_table_scaled_pct.mds.df$NMDS2), 
           label = paste("p <", otu_table_scaled_pct.perm$aov.tab$`Pr(>F)`[1], "(PERMANOVA)", sep=" ")) + theme_classic() +
  theme(plot.title = element_text(size=15, face="bold", hjust=0.5), plot.subtitle = element_text(size=10, hjust=0.5), 
        axis.title = element_text(size=10, face="bold"), axis.text = element_text(size=8, face="bold"), legend.position="bottom", axis.line = element_line(size = 0.3),
        legend.title = element_text(size=13, face="bold"), legend.text = element_text(size = 11))
#ggsave(file = "figs/otu_table_scaled_pct_mds.pdf", width = 10, height = 6, units = "in")
